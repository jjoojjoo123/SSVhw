...
while i<=n do
	...
	(*$\{i \leq n \land M \geq 0\}$*)
	i := i+1
	(*$\{i \leq n+1 \land M \geq 0\}$*)
od;
(*$\{i = n+1 \land M \geq 0\}$*)
if M=0 then
		(*$\{i = n+1 \land M = 0\}$*)
		Majority := -1
		(*$\{i = n+1 \land M = 0 \land Majority = -1\}$*)
	else
		(*$\{i = n+1 \land M > 0\}$*)
		Count := 0;
		(*$\{i = n+1 \land M > 0 \land Count = 0\}$*)
		i := 1;
		(*$\{i = 1 \land M > 0 \land Count = 0\}$*)
		while i<=n do
		...
		od;
		(*$\{i = n+1 \land M > 0 \land Count \geq 0\}$*)
		if Count>n/2 then
					(*$\{i = n+1 \land M > 0 \land Count>\frac{n}{2}\}$*)
					Majority := C
					(*$\{i = n+1 \land M > 0 \land Count>\frac{n}{2} \land Majority = C\}$*)
				else
					(*$\{i = n+1 \land M > 0 \land 0 \leq Count \leq \frac{n}{2}\}$*)
					Majority := -1
					(*$\{i = n+1 \land M > 0 \land 0 \leq Count \leq \frac{n}{2} \land Majority = -1\}$*)
		fi
		(*$\{i = n+1 \land M > 0 \land 0 \leq Count \leq \frac{n}{2}\}$*)
fi
(*$\{i = n+1 \land M \geq 0\}$*)
